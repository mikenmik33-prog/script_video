...
def _build_script_prompt(topic: str, scene_count: int, language: str) -> str:
    ...
    return (
        ...
        "ОБОВ'ЯЗКОВО має відповідати змісту voice_text саме цієї сцени - "
        "якщо в тексті йдеться про конкретний факт, подію, об'єкт чи "
        "деталь, картинка повинна показувати САМЕ ЇЇ, а не загальну "
        "абстрактну ілюстрацію теми відео. Наприклад, якщо voice_text "
        "розповідає про зниклий корабель - на картинці має бути саме "
        "корабель (чи його слід), а не просто хвилі. Кожна сцена повинна "
        ...
        "композиція (9:16), обов'язково кінематографічний стиль: драматичне "
        "освітлення, глибина кадру, якість кінокадру (film still), а не "
        "проста ілюстрація.\n"
